...
public class StoreTest extends ReadConfigUtility {
	...
	public void searchOrders() {
		...
		//expect().statusCode(200).and().when().get(baseEndpoint + "/api/v3/store/inventory");
		
		Response response = given()
                .when()
                .get(baseEndpoint + "/api/v3/store/inventory")
                .then()
                .extract().response();
	...
	public void searchOrderByID() {
		...
		expect().statusCode(200).and().when().get(baseEndpoint + "/api/v3/store/order/{orderId}", orderId);
	}
}
